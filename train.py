    ...
    for it, (images, labels, names) in enumerate(train_loader):
        ...
        if accIter['train'] % 50 == 0:
            if view_figure:
                ...
                groundtruth_tensor = labels.unsqueeze(1) * scale  # mini_batch*288*512 -> mini_batch*1*288*512
                ...
                depth_tensor = images[:,3:4]  # mini_batch*288*512 -> mini_batch*1*288*512
                depth_tensor = torch.cat((depth_tensor, depth_tensor, depth_tensor), 1)  # change to 3-channel for visualization
                depth_images = vutils.make_grid(depth_tensor, nrow=8, padding=10)
                writer.add_image('Train/depth_images', depth_images, accIter['train'])

                mask_tensor = images[:,4:]  # mini_batch*288*512 -> mini_batch*1*288*512
                mask_tensor = torch.cat((mask_tensor, mask_tensor, mask_tensor), 1)  # change to 3-channel for visualization
                mask_images = vutils.make_grid(mask_tensor, nrow=8, padding=10)
                writer.add_image('Train/mask_images', mask_images, accIter['train'])

                predicted_tensor = logits.argmax(1).unsqueeze(1) * scale # mini_batch*args.n_class*288*512 -> mini_batch*288*512 -> mini_batch*1*288*512
                predicted_tensor = torch.cat((predicted_tensor, predicted_tensor, predicted_tensor),1) # change to 3-channel for visualization, mini_batch*1*288*512
...
def testing(epo, model, test_loader):
    ...
    label_list = ["unlabeled", "road", "negative"]
    ...
    if epo==0:
        with open(testing_results_file, 'w') as f:
            ...
            f.write("# epoch: unlabeled, road, negative,  average(nan_to_num). (Pre %, Acc %, IoU %, F1 %)\n")
...
if __name__ == '__main__':
    ...
    train_dataset = RGB_Depth_Fusion_dataset(data_dir=args.data_dir, split='train', transform=augmentation_methods,threshold=0)
    val_dataset  = RGB_Depth_Fusion_dataset(data_dir=args.data_dir, split='val',threshold=0)
    test_dataset = RGB_Depth_Fusion_dataset(data_dir=args.data_dir, split='test',threshold=0)
